...
class MovieDriver{
	...
	while(!operation.equals("exit")){
	    ...
	    System.out.println("\tsortByYear:\tsort all movies by year.");
	    System.out.println("\tsortByName:\tsort all movies by name.");
	    System.out.println("\tsearchBS:\tsearch all movies by name using binary search.");
	    System.out.println("\tsearchRec:\tsearch all movies by name recursively.");
		...
		break;
	    case "searchBS":
		appSearchByNameBS(app);
		break;
	    case "searchRec":
		appSearchByNameRec(app);
		break;
	    case "exit":
		return;
	    case "print":
    ...
    public static void appSearchByNameBS(MovieApp app) {
	System.out.println("NOTE: MovieApp must be sorted by name");
	System.out.println("Which name would you like to search by?");
	String name = IO.readString();
	Movie m = app.searchMovieByName(name);
	if (m == null) {
	    System.out.println("Movie " + m + " not found");
	} else {
	    System.out.println("Found: " + m);
	}
    }

    public static void appSearchByNameRec(MovieApp app) {
	System.out.println("Which name would you like to search by?");
	String name = IO.readString();
	Movie m = MovieApp.searchMovieByName(name,app.getMovies(),0,app.getNumberOfItems()-1);
	if (m == null) {
	    System.out.println("Movie " + m + " not found");
	} else {
	    System.out.println("Found: " + m);
